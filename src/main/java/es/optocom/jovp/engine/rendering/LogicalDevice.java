...
public class LogicalDevice {
    ...
    private void createLogicalDevice(long surface, VkPhysicalDevice physicalDevice) {
        try (MemoryStack stack = stackPush()) {
            ...
            if(validationLayers) createInfo.ppEnabledLayerNames(asPointerBuffer(VALIDATION_LAYERS));
            PointerBuffer pDevice = stack.pointers(VK_NULL_HANDLE);
            int result = vkCreateDevice(physicalDevice, createInfo, null, pDevice);
            if (result != VK_SUCCESS)
                throw new AssertionError("Failed to create logical device: " + translateVulkanResult(result));
    ...
    private void createDescriptorSetLayout() {
        try (MemoryStack stack = stackPush()) {
            ...
            if (result != VK_SUCCESS)
                throw new AssertionError("Failed to create descriptor set layout: " + translateVulkanResult(result));
