package es.optocom.opi.jovp.engine.rendering;

import es.optocom.opi.jovp.engine.Items;
import es.optocom.opi.jovp.engine.Window;
import es.optocom.opi.jovp.engine.structures.ViewMode;
...
public class VulkanManager {
    ...
    public void drawFrame() {
        try (MemoryStack stack = stackPush()) {
            ...
            VK10.vkWaitForFences(VulkanSetup.logicalDevice.device, thisFrame.pFence(), true, VulkanSetup.UINT64_MAX);
            int result = KHRSwapchain.vkAcquireNextImageKHR(VulkanSetup.logicalDevice.device, VulkanSetup.swapChain.swapChain, VulkanSetup.UINT64_MAX,
            ...
            if(imagesInFlight.containsKey(imageIndex))
                VK10.vkWaitForFences(VulkanSetup.logicalDevice.device, imagesInFlight.get(imageIndex).fence(), true, VulkanSetup.UINT64_MAX);
            ...
            VK10.vkResetFences(VulkanSetup.logicalDevice.device, thisFrame.pFence());
            result = VK10.vkQueueSubmit(VulkanSetup.logicalDevice.graphicsQueue, submitInfo, thisFrame.fence());
    ...
    public void computeFieldOfView() {
        double xHalfSize = VulkanSetup.window.getPixelWidth() * VulkanSetup.window.getScaledWidth() / 2 / VulkanSetup.distance;
        double yHalfSize = VulkanSetup.window.getPixelHeight() * VulkanSetup.window.getScaledHeight() / 2 / VulkanSetup.distance;
    ...
    private void createSurface() {
        try (MemoryStack stack = stackPush()) {
            ...
            int result = GLFWVulkan.glfwCreateWindowSurface(VulkanSetup.instance, VulkanSetup.window.getHandle(), null, pSurface);
            if (result != VK_SUCCESS)
    ...
    private void destroyInstance() {
        if(VulkanSetup.validationLayers) VulkanSetup.destroyDebugUtilsMessengerEXT();
    ...
    private void enumerateSuitablePhysicalDevices() {
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            VK10.vkEnumeratePhysicalDevices(VulkanSetup.instance, deviceCount, null);
            if (deviceCount.get(0) == 0)
                throw new RuntimeException("Failed to find GPUs with Vulkan support");
            PointerBuffer ppPhysicalDevices = stack.mallocPointer(deviceCount.get(0));
            VK10.vkEnumeratePhysicalDevices(VulkanSetup.instance, deviceCount, ppPhysicalDevices);
            for (int i = 0; i < ppPhysicalDevices.capacity(); i++) {
                VkPhysicalDevice device = new VkPhysicalDevice(ppPhysicalDevices.get(i), VulkanSetup.instance);
                if (VulkanSetup.isDeviceSuitable(VulkanSetup.surface, device)) VulkanSetup.physicalDevices.add(device);
            }
        }
        if (VulkanSetup.physicalDevices.size() == 0) throw new RuntimeException("Failed to find a suitable GPU");
    }
    ...
    private void setPerspective() {
        ...
        VulkanSetup.projection.identity().perspective(VulkanSetup.fovy, (float) aspect, VulkanSetup.Z_NEAR, VulkanSetup.Z_FAR, true);
    }
    ...
    private void createSyncObjects() {
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            for (int i = 0; i < VulkanSetup.MAX_FRAMES_IN_FLIGHT; i++) {
                if (VK10.vkCreateSemaphore(VulkanSetup.logicalDevice.device, semaphoreInfo,
                        null, pImageAvailableSemaphore) != VK_SUCCESS
                        || VK10.vkCreateSemaphore(VulkanSetup.logicalDevice.device, semaphoreInfo,
                        null, pRenderFinishedSemaphore) != VK_SUCCESS
                        || VK10.vkCreateFence(VulkanSetup.logicalDevice.device, fenceInfo, null, pFence) != VK_SUCCESS) {
