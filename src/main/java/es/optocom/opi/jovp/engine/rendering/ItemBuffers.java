...
class ItemBuffers {
    ...
    private void createTextureImage(@NotNull Texture texture) {
        ...
        textureImageView = VulkanSetup.createImageView(VulkanSetup.logicalDevice.device, textureImage, VulkanSetup.COLOR_FORMAT,
    ...
    private void createTextureSampler(@NotNull Texture texture) {
        try (MemoryStack stack = stackPush()) {
            ...
            int result = VK10.vkCreateSampler(VulkanSetup.logicalDevice.device,
    ...
    private void createDescriptorPool() {
        try (MemoryStack stack = stackPush()) {
            ...
            int result = VK10.vkCreateDescriptorPool(VulkanSetup.logicalDevice.device, poolInfo, null, pDescriptorPool);
            if (result != VK_SUCCESS)
    ...
    private void createDescriptorSets() {
        try (MemoryStack stack = stackPush()) {
            ...
            int result = VK10.vkAllocateDescriptorSets(VulkanSetup.logicalDevice.device, allocInfo, pDescriptorSets);
            if (result != VK_SUCCESS)
    ...
    void updateUniforms(int imageIndex, int eye) {
        ...
        try(MemoryStack stack = stackPush()) {
            PointerBuffer data = stack.mallocPointer(1);
            vkMapMemory(VulkanSetup.logicalDevice.device, uniformBuffersMemory.get(imageIndex), 0, VulkanSetup.UNIFORM_SIZEOF, 0, data);
            {
